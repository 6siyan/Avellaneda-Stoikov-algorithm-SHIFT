...
def strategy(trader: shift.Trader, ticker: str, endtime):
    ...
    while (i):
        ...
        limit_buy = shift.Order(shift.Order.Type.LIMIT_BUY, ticker, 1, midprice-0.05)
        print(f"ticker: {ticker}, type: {limit_buy.type}, price: {midprice-0.05}")
        trader.submit_order(limit_buy)

        #order = shift.Order(shift.Order.Type.LIMIT_SELL, ticker, order_size)
        #print(f"ticker: {ticker}, type: {order.type}, price: {midprice}")
        limit_sell = shift.Order(shift.Order.Type.LIMIT_SELL, ticker, 1, midprice+0.05)
        print(f"ticker: {ticker}, type: {limit_sell.type}, price: {midprice+0.05}")
