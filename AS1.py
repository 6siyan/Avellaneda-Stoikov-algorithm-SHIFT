...
class MM_Model_Parameters:
    ...
    def __init__(self, lambda_m, lambda_p, kappa_m, kappa_p, delta, 
                 phi, alpha, q_min, q_max, T, cost, rebate, tick, epsilon_m=0, epsilon_p=0):
...
lambda_m = 50
  # Number of market order arrivals per minute on the bid
...
cost = 0.003   # trading cost per market order
rebate = 0.002
tick = 0.5
...
parameters = MM_Model_Parameters(lambda_m, lambda_p ,kappa_m, kappa_p, delta,
                                   phi, alpha, q_min, q_max, T, cost, rebate,cost, rebate, tick)
    
impulses, model = AS3P_Finite_Difference_Solver.solve(parameters, N_steps=n)

#print(model.t_grid)
#print(model.q_grid)
# print(model.m_q_grid)
#print(model.t_grid)
#print(model.m_l_m[1][1])
#for i in range(9):
#    print(model.m_l_m[i][250])
#print(model)
#print(model.m_l_p[0])
#print(model.m_t_grid)[0]
#t_idx = filter(lambda x: x >= t, self.m_t_grid)[0]
#print (model.get_l_minus_q_t(0,0.01))
#print (model.get_l_plus_q_t(0, 0.01)) # it shoule be same, cause all parameters are the same.